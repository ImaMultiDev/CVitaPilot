      ...
      <Card className="p-8">
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {essentialElements.map((element, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`${colorClasses.bg} p-6 rounded-xl border border-gray-200 dark:border-gray-600`}
                  >
                    ...
                    <ul className={`text-sm ${colorClasses.text} space-y-2`}>
                      {element.items.map((item, idx) => (
                        <li key={idx} className="flex items-start gap-2">
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${colorClasses.text} mt-2 flex-shrink-0`}
                          ></div>
                          <span>{item}</span>
                        </li>
                      ))}
          ...
          <div id="diseno-formato">
            ...
            <div className="bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20 p-8 rounded-xl border border-yellow-200 dark:border-yellow-600">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {designElements.map((element, index) => (
                  ...
                  >
                    <h5 className="font-semibold text-yellow-800 dark:text-yellow-300 mb-3">
                      {element.title}
                    </h5>
